...
! :: ----------------------------------------------------------
  ...
  subroutine FORT_SETAREA(DIMS(reg),area,DIMS(area),offset,dx,dir,coord)
    ...
    if (coord .eq. 0) then
       ...
       if (dir .eq. 0) then
          fa = dx(2)*dx(3)*dx(4)*dx(5)
       else if (dir .eq. 1) then
          fa = dx(1)*dx(3)*dx(4)*dx(5)
       else if (dir .eq. 2) then
          fa = dx(1)*dx(2)*dx(4)*dx(5)
       else if (dir .eq. 3) then
          fa = dx(1)*dx(2)*dx(3)*dx(5)
       else if (dir .eq. 4) then
          fa = dx(1)*dx(2)*dx(3)*dx(4)
       else
...
! :: ----------------------------------------------------------
  ...
  subroutine FORT_SETDLOGA(dloga,DIMS(dloga),offset,dx,dir,coord)
    ...
    if (coord .eq. 0) then
       ...
       write(6,*)' Non-cartesian not allowed in 5D '
       call bl_abort(" ")
